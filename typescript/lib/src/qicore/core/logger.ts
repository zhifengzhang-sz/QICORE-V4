...
import { type Result, Result as ResultClass, failure, isSuccess, success } from "../base/result";
...
// ============================================================================
// StructuredLogger Class for Test Compatibility
// ============================================================================

/**
 * StructuredLogger class interface for test compatibility
 * Expects constructor with config including level, component, prettyPrint
 */
export class StructuredLogger {
  ...
  constructor(config: {
    ...
    const loggerConfig: LoggerConfig = {
      level: config.level,
      enableColors: config.prettyPrint !== false,
      enableTimestamp: true,
      timestampFormat: "iso",
      output: "winston",
      prefix: config.component,
      enableConsole: true,
    };

    const result = create(loggerConfig);
    if (isSuccess(result)) {
      this.logger = result.right;
    } else {
      // Fallback to silent logger if creation fails
      const fallback = createSilent();
      this.logger = isSuccess(fallback) ? fallback.right : createSilentLogger();
    }
  }

  /**
   * Info logging method returning Result
   */
  info(message: string, context?: Record<string, unknown>): ResultClass<void> {
    try {
      this.logger.info(message, context);
      return ResultClass.success(undefined);
    } catch (error) {
      return ResultClass.failure(
        createQiError("LOGGER_ERROR", `Failed to log message: ${error}`, "SYSTEM", {
          message,
          context,
          error: String(error),
        })
...
// Helper to create a bare minimum logger
function createSilentLogger(): Logger {
  return {
    ...
    getConfig: () => createSilentLoggerConfig(),
    getLevel: () => LogLevel.FATAL,
  };
}
