...
export const merge = (left: ConfigData, right: ConfigData): ConfigData => {
  ...
  for (const [key, value] of Array.from(right.data)) {
...
// ============================================================================
// Configuration Class for Test Compatibility
// ============================================================================

/**
 * Configuration class interface that provides constructor with schema
 * and loadFromObject method for compatibility with test expectations
 */
export class Configuration {
  ...
  /**
   * Load configuration from object with validation
   */
  loadFromObject(obj: Record<string, unknown>): ResultClass<unknown> {
    try {
      // Parse the object with the provided schema
      const parsed = (this.schema as { parse: (obj: unknown) => unknown }).parse(obj);

      // Store the parsed object directly for test compatibility
      this.data = parsed;
      return ResultClass.success(parsed);
    } catch (error) {
   ...
   */
  get(): ResultClass<unknown> {
    if (this.data) {
      return ResultClass.success(this.data);
    }
    return ResultClass.failure(
      createQiError("CONFIG_NOT_LOADED", "Configuration not loaded", "SYSTEM", {})
    );
  }
}
