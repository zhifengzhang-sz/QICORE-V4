...
import { QiError, QiErrorGuards, ErrorCollector } from "../../src/qicore/base/index.js";
...
describe("QiError", () => {
  ...
  test("should get error chain", () => {
    ...
    const error2 = QiError.chain(
      QiError.networkError("Error 2", "url"),
      error1
    );
    const error3 = QiError.chain(
      QiError.timeoutError("Error 3", "op", 30),
      error2
    );
  ...
  test("should find error in chain", () => {
    ...
    const network = QiError.chain(
      QiError.networkError("Network", "url"),
      validation
    );
